...
def create_orders_table(session):
    _ = session.sql("CREATE TABLE PUBLIC.ORDERS LIKE HARMONIZED.POS_FLATTENED_V").collect()
    _ = session.sql("ALTER TABLE PUBLIC.ORDERS ADD COLUMN META_UPDATED_AT TIMESTAMP").collect()

def create_orders_stream(session):
    _ = session.sql("CREATE STREAM PUBLIC.ORDERS_STREAM ON TABLE PUBLIC.ORDERS").collect()

def merge_order_updates(session):
    _ = session.sql('ALTER WAREHOUSE HOL_WH SET WAREHOUSE_SIZE = XLARGE WAIT_FOR_COMPLETION = TRUE').collect()

    source = session.table('HARMONIZED.POS_FLATTENED_V_STREAM')
    target = session.table('PUBLIC.ORDERS')
...
def main(session: Session) -> str:
    ...
    if not table_exists(session, schema='PUBLIC', name='ORDERS'):
        create_orders_table(session)
        create_orders_stream(session)

    # Process data incrementally
    merge_order_updates(session)
#    session.table('PUBLIC.ORDERS').limit(5).show()
